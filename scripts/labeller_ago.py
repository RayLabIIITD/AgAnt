...
if len(filenames)<=0:
    ...
    with open("labels/labels.json") as fp:
        labels=json.load(fp)
    x+=len(labels)
    for filename in glob.glob('pdb_files/ago_header/*.pdb'):
        target=""
        text=""
        title=""
        header=""
    ...
    df.to_excel("labels/labels.xlsx")
else:
    x=0
    df = pd.read_excel (r'labels/labels_ago.xlsx')
    df.drop_duplicates(subset ="PDB_ID", 
    ...
    for n,i in df.iterrows():
        ...
        if i["Label"]!=i["Label"]:
            ...
            text=i["PUBMED_ABS"]
            l1=['agonist','antagonist','agonist,','agonist.']
            result = " ".join(colored(t,'white','on_red') if t in l1 else t for t in text.split())
            print(result)
            #print(i["PUBMED_ABS"])
            labell=input()
            df.at[n,"Label"]=labell
            df.to_excel("labels/labels_ago.xlsx")
        elif i["Label"]==0:
            a+=1
        elif i["Label"]==1:
            b+=1
